...
class SamsungRAWImage:
    @staticmethod
    def load(path, colored_image):
        im_raw = cv2.imread('{}/im_raw.png'.format(path), cv2.IMREAD_UNCHANGED)
        if not colored_image:
            im_raw = (cv2.cvtColor(im_raw, cv2.COLOR_BGR2GRAY, dstCn=1)).astype(np.int16)
        else:
    ...
    def get_image_data(self, substract_black_level=False, white_balance=False, normalize=False, colored_image=False):
...
class CanonImage:
    ...
    def load(path, colored_image):
        im_raw = cv2.imread('{}/im_raw.png'.format(path), cv2.IMREAD_UNCHANGED)
        if not colored_image:
            im_raw = cv2.cvtColor(im_raw, cv2.COLOR_BGR2GRAY).astype(np.int16)
        else:
            im_raw = np.transpose(im_raw, (2, 0, 1)).astype(np.int16)
        im_raw = torch.from_numpy(im_raw)
        meta_data = pkl.load(open('{}/meta_info.pkl'.format(path), "rb", -1))
    ...
    def __init__(self, im_raw, black_level, cam_wb, daylight_wb, rgb_xyz_matrix, exif_data, crop_info=None, colored_image=False):
    ...
    def shape(self, colored_image):
        if colored_image:
            ...
            shape = (self.im_raw[0], self.im_raw[1])
        return shape
...
class BurstSRDataset(torch.utils.data.Dataset):
    ...
    def _get_raw_image(self, burst_id, im_id):
        raw_image = SamsungRAWImage.load('{}/{}/samsung_{:02d}'.format(self.root, self.burst_list[burst_id], im_id), self.colored_image)
        return raw_image

    def _get_gt_image(self, burst_id):
        canon_im = CanonImage.load('{}/{}/canon'.format(self.root, self.burst_list[burst_id]), self.colored_image)
        return canon_im
    ...
    def __getitem__(self, index):
        ...
        if frames[0].shape(self.colored_image)[-1] != self.crop_sz:
        ...
        burst_image_data = [im.get_image_data(normalize=True, substract_black_level=self.substract_black_level,
                                              ...
                                              colored_image=self.colored_image) for im in frames]
        # Convert to tensor
        gt_image_data = gt.get_image_data(normalize=True, white_balance=self.white_balance,
                                          ...
                                          colored_image=self.colored_image)
        ...
        if self.random_flip:
            ...
            if self.colored_image:
                ...
                gt_image_data = F.pad(gt_image_data.unsqueeze(0), [p for p in pad], mode='replicate').squeeze(0)
...
# def main():
#     burstSR = BurstSRDataset(root='C:/Users/Default.DESKTOP-FRPMN5G/Documents/Master/UC/project/burstsr_dataset')
#     burst, frame_gt, meta_info_burst, meta_info_gt = burstSR.__getitem__(10)
#     print(f'Burst: {burst.shape}')
#     print(f'Framegt: {frame_gt.shape}')
#     plt.show()
